...
gulp.task('replaceFileKey', () => {
  gulp.src(['./src/scripts/main.js'])
    .pipe(replaceString('goldenrose', pkg.name))
    .pipe(gulp.dest((data) => data.base));

  gulp.src(['./src/index.html'])
    .pipe(replaceString('keyboard_theme_golden_rose', pkg.packageName))
    .pipe(gulp.dest((data) => data.base));

  gulp.src(['./src/scripts/constValue.js'])
    .pipe(replaceString('goldenrose', pkg.name))
    .pipe(replaceString('keyboard_theme_golden_rose', pkg.packageName))
    .pipe(gulp.dest((data) => data.base));
})
// gulp.task('replacepackageKey', () => {
//   gulp.src(['./src/index.html', './src/scripts/constValue.js'])
//     .pipe(replaceString('keyboard_theme_golden_rose', pkg.packageName));
// })
...
loadTask.makeTaskWithDependency(
  'compressJs', ['webpackWithMoveJs'],
  projectConfig_usa,
  'compressJsWithWebpack'
)
loadTask.makeTaskWithDependency(
  'cssMin', ['sass'],
  projectConfig_usa,
...
gulp.task('webp', () => {
  gulp.src('./src/raw/banner.*(jpg|png)').pipe(gulp.dest('./src/images/'))
  return gulp
    .src('./src/raw/**')
    .pipe(
      webp({
  ...
  shareConfig.supportLanguages.forEach(lang => {
    gulp
      ...
      .pipe(
        replaceString(
          '@og:description',
          shareConfig.translateMap.shareText[lang]
        )
      )
      .pipe(
        replaceString(
          '@og:image',
          'http://ime.cdn.cootekservice.com/default/webpage/' +
          projectConfig_usa.name +
          '/images/banner.jpg'
        )
      )
